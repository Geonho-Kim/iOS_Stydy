...
struct Store{
    ...
    let deliberryRange = 2.0
    
    func isDeliberable(userLoc: Location)->Bool{
        let distanceToStore = distance(current: userLoc, target: loc)
        return distanceToStore < deliberryRange
    }
...
func printClosestStore(currentLocation: Location, stores:[Store]) {
    ...
    for store in stores {
        ...
        if closestStoreDistance == distanceToStore {
            ...
            isDeliberable = store.isDeliberable(userLoc: currentLocation)
        }
    }
    print("Closest store: \(closestStoreName), isDeliverable: \(isDeliberable)")
}
//
